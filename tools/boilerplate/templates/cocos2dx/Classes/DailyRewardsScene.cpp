...
Scene* DailyRewardsScene::createScene() {
    return DailyRewardsScene::create();
}

bool DailyRewardsScene::init() {
    if (!Scene::init()) return false;
    
    Size visible = Director::getInstance()->getVisibleSize();
    Vec2 origin = Director::getInstance()->getVisibleOrigin();
    
    auto label = Label::createWithSystemFont("Daily Rewards", "Arial", 36);
