...
When using custom registries, you must also set `default-registry` (e.g. `"kind": "builtin"` with a baseline commit). See [vcpkg registries](https://learn.microsoft.com/vcpkg/users/registries). Then run `vcpkg install intelliversex-cpp`.
...
1. Ensure a release tag (e.g. `v1.5.0`) exists in the IntelliVerseX repo.
