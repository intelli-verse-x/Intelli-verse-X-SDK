...
- **`.agents/skills/`** — project-level mirror of [`skills/`](skills/README.md) for clients that discover `~/.agents/skills/` or `<repo>/.agents/skills/`; [`tools/scripts/install-agents-skills.ps1`](tools/scripts/install-agents-skills.ps1) / [`.sh`](tools/scripts/install-agents-skills.sh) copy into the user folder; [`.gitignore`](.gitignore) allows `.agents/skills/` only
